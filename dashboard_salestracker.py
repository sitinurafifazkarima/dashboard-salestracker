...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    "🏆 KPI & Summary", "📈 Pipeline & Forecast", "🔥 Aktivitas & Root Cause", "🎯 Prioritas & Rekomendasi", "🔍 Analisis Lanjutan", "💡 Insight Otomatis"
])

with tab1:
    st.subheader("KPI Utama & Ringkasan")
    ...
    ketercapaian_target.plot(kind='bar', color='#5DADE2', edgecolor='black', ax=ax1)
    ...
    conversion_rate_sales.sort_values(ascending=False).plot(kind='bar', color='#F5B041', edgecolor='black', ax=ax2)
...
with tab2:
    st.subheader("Pipeline Value per Tahap & Forecast Revenue")
    if 'Progress' in filtered_df.columns:
        ...
        pipeline_value.plot(kind='bar', color='#2874A6', edgecolor='black', ax=ax)
        ax.set_ylabel('Total Nilai Kontrak (Rp)')
        ax.set_title('Pipeline Value per Tahap')
        ax.set_xticklabels(ax.get_xticklabels(), rotation=30)
        st.pyplot(fig)
        # Forecast revenue
        deal_value = last_progress[last_progress['Status_Kontrak'].str.lower() == 'deal']['Nilai_Kontrak'].sum()
        # Probabilitas deal dummy (bisa diisi model prediksi)
        prob_value = last_progress[last_progress['Status_Kontrak'].str.lower() != 'deal']['Nilai_Kontrak'].sum() * 0.3
        total_forecast = deal_value + prob_value
        st.info(f"Total nilai deal saat ini: Rp {deal_value:,.0f}")
        st.info(f"Forecast revenue (deal + estimasi non-deal): Rp {total_forecast:,.0f}")
        st.success(f"Pipeline value terbesar ada di tahap: {pipeline_value.idxmax()}")
    else:
...
with tab3:
    ...
    kunjungan_per_sales = filtered_df.groupby('Nama_Sales')['ID_Kunjungan'].count()
    nilai_kontrak_per_sales = deal_df.groupby('Nama_Sales')['Nilai_Kontrak'].sum()
    target_sales_per_sales = filtered_df.groupby('Nama_Sales')['Target_Sales'].sum()
    ketercapaian_target = (nilai_kontrak_per_sales / target_sales_per_sales).fillna(0)
